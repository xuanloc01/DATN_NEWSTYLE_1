...
$product = array();
$sql = "select *from `tbl_product` where `id_cat`={$id_cat} limit $start,6";
$result = mysqli_query($conn, $sql);
while ($row = mysqli_fetch_assoc($result)) {
    $product[] = $row;
}
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12 text-center">
                <?php
                $sql = "select *from `tbl_product` where `id_cat`={$id_cat}";
                $result = mysqli_query($conn, $sql);
                $sum_pro = mysqli_num_rows($result);
                $sum_page = ceil($sum_pro / 6);
                ...
                <ul class="page">
                    ...
                    for ($i = 1; $i < $sum_page; $i++) {
                        ...
                        <li><a <?php if ($num_page == $i) {
                          ...
                          ?>&act=product_cat&id_cat=<?php echo $item['id_cat'] ?>&page=<?php echo $i ?>">
                                <?php echo $i ?>
                            </a></li>
                        <?php
                    }
